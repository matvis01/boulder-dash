...
bool Game::canMoveLeft()
{
	if (player.playerPosTile.x > 0) // cant go outside of map
	{
		...
		{
			if (player.playerPosTile.x > 2 and player.playerPosTile.x < level.mapSizeX + 2 and this->level.tiles[player.playerPosTile.x - 1][player.playerPosTile.y]->getName() == Name::rock and this->level.tiles[player.playerPosTile.x - 2][player.playerPosTile.y] == nullptr)
			{// single rock on right
				return true;
			}
			if (player.playerPosTile.x >= 1 and this->level.tiles[player.playerPosTile.x - 1][player.playerPosTile.y]->isPassable)
			{
				return true;
			}
			else 
